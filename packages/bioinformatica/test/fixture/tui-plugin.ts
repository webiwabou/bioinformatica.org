...
import type { HostPluginApi } from "@bioinformatica/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createBioinformaticaClient({
  ...
  let selected = opts.theme?.selected ?? "bioinformatica"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "bioinformatica.default",
