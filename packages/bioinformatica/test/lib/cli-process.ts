// Subprocess test harness for the bioinformatica CLI. Spawns the real binary against
...
//
// Configuration flows through bioinformatica's built-in test affordances:
//   - BIOINFORMATICA_CONFIG_CONTENT      : provider config inline, no files to find
//   - BIOINFORMATICA_TEST_HOME           : pins os.homedir() → tmpdir
//   - BIOINFORMATICA_DISABLE_PROJECT_CONFIG : skip walking up for bioinformatica.json
//   - BIOINFORMATICA_PURE                : skip external plugin discovery + install
//   - BIOINFORMATICA_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `bioinformatica.run` is fully wired. The shape supports adding more
// builders (`bioinformatica.serve(opts)`, `bioinformatica.acp(opts)`, `bioinformatica.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of BioinformaticaCli.
...
const bioinformaticaRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(bioinformaticaRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    BIOINFORMATICA_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    BIOINFORMATICA_CONFIG_CONTENT: configJson,
    BIOINFORMATICA_DISABLE_PROJECT_CONFIG: "1",
    BIOINFORMATICA_PURE: "1",
    BIOINFORMATICA_DISABLE_AUTOUPDATE: "1",
    BIOINFORMATICA_DISABLE_AUTOCOMPACT: "1",
    BIOINFORMATICA_DISABLE_MODELS_FETCH: "1",
    BIOINFORMATICA_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `bioinformatica run`. New flags should
...
// `bioinformatica serve` is a long-lived process — it never exits on its own.
...
// `bioinformatica acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type BioinformaticaCli = {
  ...
  // Spawn `bioinformatica serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `bioinformatica acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly bioinformatica: BioinformaticaCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("bioinformatica.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          BIOINFORMATICA_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("bioinformatica.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("bioinformatica.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "bioinformatica server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `bioinformatica serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("bioinformatica.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const bioinformatica: BioinformaticaCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, bioinformatica })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `bioinformatica.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "bioinformatica") {
...
// every call site. Use this for any test that needs the bioinformatica CLI fixture.
//
...
// resources (e.g. `bioinformatica.serve`) without an extra `Effect.scoped` wrapper —
