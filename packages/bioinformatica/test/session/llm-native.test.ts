...
import { LLMEvent, ToolFailure } from "@bioinformatica/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@bioinformatica/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("bioinformatica"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("bioinformatica") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("bioinformatica") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, bioinformatica, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored bioinformatica auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("bioinformatica") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("bioinformatica"),
