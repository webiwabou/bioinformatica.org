...
import { SessionV1 } from "@bioinformatica/core/v1/session"
...
import { LLMEvent, LLMResponse } from "@bioinformatica/llm"
import { RequestExecutor } from "@bioinformatica/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.BIOINFORMATICA_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["bioinformatica", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BIOINFORMATICA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("BIOINFORMATICA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["bioinformatica", "native", "oauth", "tool-loop"],
  ...
  {
    id: "bioinformatica-proxy",
    name: "Bioinformatica proxy",
    providerID: ProviderV2.ID.bioinformatica,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["bioinformatica", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.BIOINFORMATICA_RECORD_CONSOLE_TOKEN && process.env.BIOINFORMATICA_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.bioinformatica,
        name: "Example Gateway",
        env: ["BIOINFORMATICA_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.BIOINFORMATICA_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.BIOINFORMATICA_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.BIOINFORMATICA_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["bioinformatica", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BIOINFORMATICA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("BIOINFORMATICA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("BIOINFORMATICA_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set BIOINFORMATICA_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the bioinformatica LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "bioinformatica.json"),
      JSON.stringify({ $schema: "https://bioinformatica.org/config.json", ...scenario.config(model) }),
    ),
  )
