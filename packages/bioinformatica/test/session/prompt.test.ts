...
import { SessionMessageTable } from "@bioinformatica/core/session/sql"
...
import { LocationServiceMap, locationServiceMapLayer } from "@bioinformatica/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://bioinformatica.org/config.json", ...config }),
  )
})
    ...
    }),
  {
    init: (directory: string) =>
      Effect.promise(async () => {
        ...
        await fs.mkdir(p.join(directory, ".bioinformatica"), { recursive: true })
        await fs.writeFile(
          p.join(directory, ".bioinformatica", "objective.json"),
          JSON.stringify({
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/bioinformatica/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("bioinformatica"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
