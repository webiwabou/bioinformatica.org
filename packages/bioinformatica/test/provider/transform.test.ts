    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".bioinformatica/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const bioinformaticaModel = {
      ...openaiModel,
      providerID: "bioinformatica",
      api: {
        id: "bioinformatica-test",
        url: "https://api.bioinformatica.org",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, bioinformaticaModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.bioinformatica?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.bioinformatica?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          bioinformatica: { itemId: "msg_bioinformatica" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              bioinformatica: { itemId: "msg_bioinformatica_part" },
    ...
    expect(result[0].providerOptions?.bioinformatica?.itemId).toBe("msg_bioinformatica")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.bioinformatica?.itemId).toBe("msg_bioinformatica_part")
