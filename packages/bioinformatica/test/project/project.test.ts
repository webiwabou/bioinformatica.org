...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const bioinformaticaFile = path.join(tmp, ".git", "bioinformatica")
      expect(yield* Effect.promise(() => Bun.file(bioinformaticaFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "bioinformatica")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "bioinformatica")
      const wrongCache = path.join(parentDir, ".git", "bioinformatica")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "bioinformatica")
      const cacheB = path.join(bareB, "bioinformatica")
      const wrongCache = path.join(parentDir, ".git", "bioinformatica")
