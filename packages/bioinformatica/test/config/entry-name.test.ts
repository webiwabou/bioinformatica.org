...
describe("configEntryNameFromPath", () => {
  ...
  // the agent key (e.g. `.config/bioinformatica/agents/build`). Anchoring at the
  ...
  test("regression #25713: caller passes relative path; parent /agent/ segment is irrelevant", () => {
    const dir = "/home/agent/.config/bioinformatica"
    const item = "/home/agent/.config/bioinformatica/agents/build.md"
  ...
  test("regression #25713: parent /agents/ segment is irrelevant", () => {
    const dir = "/srv/agents/team/.config/bioinformatica"
    const item = "/srv/agents/team/.config/bioinformatica/agents/build.md"
    const relative = posix.relative(dir, item)
    expect(configEntryNameFromPath(relative, AGENT_PREFIXES)).toBe("build")
  })
})
