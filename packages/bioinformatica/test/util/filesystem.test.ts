...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "bioinformatica.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "bioinformatica.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bioinformatica.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bioinformatica.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["bioinformatica.json", "bioinformatica.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "bioinformatica.json"),
        path.join(project, "bioinformatica.jsonc"),
      ])
    })
  })
