...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.BIOINFORMATICA_TEST ?? "")'), {
      env: {
        BIOINFORMATICA_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "BIOINFORMATICA_TEST_SHELL"], {
      shell: true,
      env: {
        BIOINFORMATICA_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("BIOINFORMATICA_TEST_SHELL=ok")
  })
