...
import type { Event, BioinformaticaClient } from "@bioinformatica/sdk/v2"
...
type PermissionReplyParams = Parameters<BioinformaticaClient["permission"]["reply"]>[0]
...
function createHarness(
  ...
  } as unknown as BioinformaticaClient
  const connection = {
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "bioinformatica-acp-permission-"))
