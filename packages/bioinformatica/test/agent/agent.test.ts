...
it.instance("plan agent denies edits except .bioinformatica/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".bioinformatica/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".bioinformatica", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.BIOINFORMATICA_TEST_HOME
      process.env.BIOINFORMATICA_TEST_HOME = test.directory
