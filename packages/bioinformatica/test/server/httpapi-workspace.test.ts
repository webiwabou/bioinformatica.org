...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-bioinformatica-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-bioinformatica-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://bioinformatica.org/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-bioinformatica-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-bioinformatica-workspace")
