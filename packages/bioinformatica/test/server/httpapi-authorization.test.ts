...
const noAuthLayer = ServerAuth.Config.configLayer({ password: Option.none(), username: "bioinformatica" })
const secretLayer = ServerAuth.Config.configLayer({ password: Option.some("secret"), username: "bioinformatica" })
...
describe("HttpApi authorization middleware", () => {
  ...
  itSecret.live("requires configured password for basic auth", () =>
    Effect.gen(function* () {
      const [missing, badPassword, good] = yield* Effect.all(
        [
          getProbe(),
          getProbe({ authorization: basic("bioinformatica", "wrong") }),
          getProbe({ authorization: basic("bioinformatica", "secret") }),
        ],
  ...
  itKitSecret.live("respects configured basic auth username", () =>
    Effect.gen(function* () {
      const [defaultUser, configuredUser] = yield* Effect.all(
        [getProbe({ authorization: basic("bioinformatica", "secret") }), getProbe({ authorization: basic("kit", "secret") })],
        { concurrency: "unbounded" },
      )
  ...
  itSecret.live("accepts auth token query credentials", () =>
    Effect.gen(function* () {
      const response = yield* HttpClient.get(`/probe?auth_token=${encodeURIComponent(token("bioinformatica", "secret"))}`)
  ...
  itSecret.live("prefers auth token query credentials over basic auth", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get(
        `/probe?auth_token=${encodeURIComponent(token("bioinformatica", "secret"))}`,
      ).pipe(HttpClientRequest.setHeader("authorization", basic("bioinformatica", "wrong")), HttpClient.execute)
  ...
  itSecret.live("preserves handler errors when basic auth succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get("/missing").pipe(
        HttpClientRequest.setHeader("authorization", basic("bioinformatica", "secret")),
        HttpClient.execute,
      )
  ...
  itSecret.live("preserves handler errors when auth token query succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClient.get(`/missing?auth_token=${encodeURIComponent(token("bioinformatica", "secret"))}`)
