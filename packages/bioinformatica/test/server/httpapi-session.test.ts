...
import { SessionInputTable, SessionMessageTable, SessionTable } from "@bioinformatica/core/session/sql"
...
const originalWorkspaces = Flag.BIOINFORMATICA_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.BIOINFORMATICA_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-bioinformatica-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-bioinformatica-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-bioinformatica-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.BIOINFORMATICA_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-bioinformatica-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "bioinformatica", "src")
        ...
        const query = new URLSearchParams({
          scope: "project",
          path: "packages/bioinformatica/src",
          directory: currentDir,
        })
        const headers = { "x-bioinformatica-directory": test.directory }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-bioinformatica-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-bioinformatica-directory": globalWorktreeSentinel, "content-type": "application/json" }
