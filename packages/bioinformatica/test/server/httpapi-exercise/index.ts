 ...
 *
 * The script intentionally isolates `BIOINFORMATICA_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "bioinformatica.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "bioinformatica.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-bioinformatica-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-bioinformatica-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "bioinformatica", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-bioinformatica-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "content-type": "application/json" },
      body: { model: { providerID: "bioinformatica", id: "big-pickle" } },
    }))
    .status(204, undefined, "none"),
  http.protected
