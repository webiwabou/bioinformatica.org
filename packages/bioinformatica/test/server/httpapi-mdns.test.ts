...
describe("HttpApi Server.listen mDNS", () => {
  test("skips publish for loopback hostnames", async () => {
    Flag.BIOINFORMATICA_SERVER_PASSWORD = "mdns-secret"
    Flag.BIOINFORMATICA_SERVER_USERNAME = "bioinformatica"
  ...
  test("publishes for non-loopback hostnames and unpublishes on stop", async () => {
    ...
    try {
      ...
      expect(published[0]!.name).toBe(`bioinformatica-${listener.port}`)
    } finally {
