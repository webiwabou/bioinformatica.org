...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/bioinformatica.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/bioinformatica.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/bioinformatica.jsonc", dir: ".bioinformatica", suggestion: "bioinformatica" },
        expected:
          'Directory ".bioinformatica" in /tmp/bioinformatica.jsonc is not valid. Rename the directory to "bioinformatica" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/bioinformatica.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/bioinformatica.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `bioinformatica models` to list available models",
      "Or check your config (bioinformatica.json) provider/model names",
    ].join("\n")
