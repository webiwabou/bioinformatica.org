...
describe("bioinformatica read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // BIOINFORMATICA_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["mcp", "list"])
        bioinformatica.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["providers", "list"])
        bioinformatica.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["models"])
        bioinformatica.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["agent", "list"])
        bioinformatica.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh BIOINFORMATICA_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["session", "list"])
        bioinformatica.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["stats"])
        bioinformatica.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under BIOINFORMATICA_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const r = yield* bioinformatica.spawn(["db", "path"])
        bioinformatica.expectExit(r, 0, "db path")
