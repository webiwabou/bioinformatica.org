...
import { BioinformaticaClient, type GlobalEvent } from "@bioinformatica/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<BioinformaticaClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<BioinformaticaClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<BioinformaticaClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<BioinformaticaClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<BioinformaticaClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://bioinformatica.test"),
...
function sdk(
  input: {
    ...
    subscribe?: BioinformaticaClient["event"]["subscribe"]
    globalEvent?: BioinformaticaClient["global"]["event"]
    promptAsync?: BioinformaticaClient["session"]["promptAsync"]
    status?: BioinformaticaClient["session"]["status"]
    messages?: BioinformaticaClient["session"]["messages"]
    children?: BioinformaticaClient["session"]["children"]
    permissions?: BioinformaticaClient["permission"]["list"]
    questions?: BioinformaticaClient["question"]["list"]
  } = {},
) {
  const client = new BioinformaticaClient()

  const subscribe: BioinformaticaClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: BioinformaticaClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: BioinformaticaClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: BioinformaticaClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: BioinformaticaClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: BioinformaticaClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: BioinformaticaClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: BioinformaticaClient["question"]["list"] = input.questions ?? (() => ok([]))
