// Subprocess integration tests for `bioinformatica run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `bioinformatica.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `BIOINFORMATICA_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("bioinformatica run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bioinformatica.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bioinformatica.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* bioinformatica.run("think", { extraArgs: ["--thinking"] })
        bioinformatica.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* bioinformatica.run("think again")
        bioinformatica.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bioinformatica.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bioinformatica.run("say hi", { format: "json" })
        bioinformatica.expectExit(result, 0)

        const events = bioinformatica.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const result = yield* bioinformatica.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bioinformatica.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bioinformatica.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, bioinformatica }) =>
      Effect.gen(function* () {
        yield* llm.tool("bash", { command: "rm -f denied-file", description: "Remove a test file" })
        yield* llm.text("continued after rejection")
        const denied = yield* bioinformatica.run("request permission", { permission: { bash: "ask" } })
        bioinformatica.expectExit(denied, 0)
        ...
        const allowed = yield* bioinformatica.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        bioinformatica.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* bioinformatica.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        bioinformatica.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, bioinformatica }) =>
      Effect.gen(function* () {
        ...
        const server = yield* bioinformatica.serve()

        const result = yield* bioinformatica.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, bioinformatica }) =>
      Effect.gen(function* () {
        const result = yield* bioinformatica.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, bioinformatica }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* bioinformatica.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
