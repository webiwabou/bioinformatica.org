...
describe("bioinformatica acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ bioinformatica }))
        ...
        expect(initialized.agentInfo?.name).toBe("Bioinformatica")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ bioinformatica })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("bioinformatica-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "bioinformatica-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.BIOINFORMATICA_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bioinformatica }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("bioinformatica-login")
