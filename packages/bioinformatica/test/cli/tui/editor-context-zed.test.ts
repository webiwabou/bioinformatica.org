...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@bioinformatica/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.BIOINFORMATICA_ZED_DB
  process.env.BIOINFORMATICA_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.BIOINFORMATICA_ZED_DB
    else process.env.BIOINFORMATICA_ZED_DB = previous
    home.mockRestore()
  }
})
