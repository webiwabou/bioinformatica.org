...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ bioinformatica }) =>
    Effect.gen(function* () {
      const result = yield* bioinformatica.spawn(["--replay-limit", "10"])

      bioinformatica.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ bioinformatica }) =>
    Effect.gen(function* () {
      const result = yield* bioinformatica.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ bioinformatica }) =>
    Effect.gen(function* () {
      const result = yield* bioinformatica.spawn(["--mini", "--port", "4096"])
