...
import { EventSequenceTable } from "@bioinformatica/core/event/sql"
...
import { WorkspaceTable } from "@bioinformatica/core/control-plane/workspace.sql"
...
const originalEnv = {
  BIOINFORMATICA_AUTH_CONTENT: process.env.BIOINFORMATICA_AUTH_CONTENT,
  BIOINFORMATICA_EXPERIMENTAL_WORKSPACES: process.env.BIOINFORMATICA_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.BIOINFORMATICA_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@bioinformatica.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.BIOINFORMATICA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=bioinformatica-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.BIOINFORMATICA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.BIOINFORMATICA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.BIOINFORMATICA_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=bioinformatica-test")
