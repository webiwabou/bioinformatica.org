...
export type PatchDeps = {
  ...
  files: (dir: string, name: "bioinformatica" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".bioinformatica")
}

function patchName(kind: Kind): "bioinformatica" | "tui" {
  if (kind === "server") return "bioinformatica"
  return "tui"
}
