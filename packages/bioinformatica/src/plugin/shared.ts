...
export const DEPRECATED_PLUGIN_PACKAGES = ["bioinformatica-openai-codex-auth", "bioinformatica-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, bioinformaticaVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(bioinformaticaVersion) || semver.major(bioinformaticaVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.bioinformatica
  if (typeof range !== "string") return
  if (!semver.satisfies(bioinformaticaVersion, range)) {
    throw new Error(`Plugin requires bioinformatica ${range} but running ${bioinformaticaVersion}`)
  }
}
