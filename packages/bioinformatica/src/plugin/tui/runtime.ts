...
import { errorData, errorMessage } from "@bioinformatica/tui/util/error"
...
import { hasTheme, upsertTheme } from "@bioinformatica/tui/context/theme"
import { Global } from "@bioinformatica/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@bioinformatica/core/util/flock"
import { Flag } from "@bioinformatica/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@bioinformatica/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@bioinformatica/core/v1/config/plugin"
import { createCommandShim } from "@bioinformatica/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@bioinformatica/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".bioinformatica"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".bioinformatica", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".bioinformatica", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.BIOINFORMATICA_PURE ? [] : pluginOrigins
    if (Flag.BIOINFORMATICA_PURE && pluginOrigins.length) {
    }
