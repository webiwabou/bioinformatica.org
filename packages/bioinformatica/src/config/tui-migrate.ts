...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from bioinformatica.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const bioinformatica = await bioinformaticaFiles(input)
  for (const file of bioinformatica) {
...
async function bioinformaticaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "bioinformatica"),
    ...(await Filesystem.findUp(["bioinformatica.json", "bioinformatica.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "bioinformatica"))
  }
  if (Flag.BIOINFORMATICA_CONFIG) files.push(Flag.BIOINFORMATICA_CONFIG)
