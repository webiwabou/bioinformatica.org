...
import { isRecord } from "@bioinformatica/tui/util/record"
...
import { TuiKeybind } from "@bioinformatica/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@bioinformatica/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old bioinformatica.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.bioinformatica`
  // folders between cwd and home, and BIOINFORMATICA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.BIOINFORMATICA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit BIOINFORMATICA_TUI_CONFIG override, if set.
  if (Flag.BIOINFORMATICA_TUI_CONFIG) {
    const configFile = Flag.BIOINFORMATICA_TUI_CONFIG
  ...
  // 4. `.bioinformatica` directories (and BIOINFORMATICA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".bioinformatica") || dir === Flag.BIOINFORMATICA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".bioinformatica") && dir !== Flag.BIOINFORMATICA_CONFIG_DIR) continue
