...
import {
  ...
  type JsonSchema,
  type LLMEvent,
} from "@bioinformatica/llm"
import type { LLMClientShape } from "@bioinformatica/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("bioinformatica"))
    return { type: "unsupported", reason: "provider is not openai, bioinformatica, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @bioinformatica/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains bioinformatica-owned. The native runtime only adapts
      // the @bioinformatica/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
