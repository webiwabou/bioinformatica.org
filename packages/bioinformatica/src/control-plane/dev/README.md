...
In a separate terminal, run a separate Bioinformática.org server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/bioinformatica/script/run-workspace-server
```

With the plugin install, you can now run Bioinformática.org and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
