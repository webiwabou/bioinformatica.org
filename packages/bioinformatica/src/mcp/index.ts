...
import { McpEvent } from "@bioinformatica/schema/mcp-event"
...
function createClient(directory: string) {
  const client = new Client({ name: "bioinformatica", version: InstallationVersion }, CLIENT_OPTIONS)
...
export class Service extends Context.Service<Service, Interface>()("@bioinformatica/MCP") {}
  ...
  Effect.gen(function* () {
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: bioinformatica mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          ...(cmd === "bioinformatica" ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        },
      })
