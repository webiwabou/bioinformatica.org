...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall bioinformatica and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Bioinformatica")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g bioinformatica",
      pnpm: "pnpm uninstall -g bioinformatica",
      bun: "bun remove -g bioinformatica",
      yarn: "yarn global remove bioinformatica",
      brew: "brew uninstall bioinformatica",
      choco: "choco uninstall bioinformatica",
      scoop: "scoop uninstall bioinformatica",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "bioinformatica", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".bioinformatica")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Bioinformatica!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# bioinformatica") || content.includes(".bioinformatica/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".bioinformatica/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".bioinformatica/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".bioinformatica"))
    ) {
      continue
    }
