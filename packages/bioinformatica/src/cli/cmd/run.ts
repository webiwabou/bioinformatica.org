import type { PermissionV1 } from "@bioinformatica/core/v1/permission"
import { FSUtil } from "@bioinformatica/core/fs-util"
// CLI entry point for `bioinformatica run` and `bioinformatica --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`bioinformatica --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`bioinformatica --mini --attach`): connects to a running
//      bioinformatica server and runs interactive mode against it.
//
...
import { createBioinformaticaClient, type BioinformaticaClient, type ToolPart } from "@bioinformatica/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<BioinformaticaClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run bioinformatica with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running bioinformatica server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to BIOINFORMATICA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to BIOINFORMATICA_SERVER_USERNAME or 'bioinformatica')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachSDK = (dir?: string) => {
        return createBioinformaticaClient({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(sdk: BioinformaticaClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: BioinformaticaClient, sessionID: string) {
      ...
      async function current(sdk: BioinformaticaClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: BioinformaticaClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: BioinformaticaClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: BioinformaticaClient) {
        ...
        async function loop(client: BioinformaticaClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createBioinformaticaClient({
        baseUrl: "http://bioinformatica.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
