...
import { BioinformaticaHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for bioinformatica serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(BioinformaticaHttpApi)
...
export const PublicApi = BioinformaticaHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "bioinformatica",
    version: "1.0.0",
    description: "bioinformatica api",
