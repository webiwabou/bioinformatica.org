...
export interface Interface {
   ...
   *
   * This exists while the old bioinformatica project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@bioinformatica/ProjectV2") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "bioinformatica")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "bioinformatica"), input.id).pipe(Effect.ignore)
    })
