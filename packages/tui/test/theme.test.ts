...
test("addTheme writes into module theme store", () => {
  ...
  expect(addTheme(name, DEFAULT_THEMES.bioinformatica)).toBe(true)
  expect(allThemes()[name]).toBeDefined()
})

test("addTheme keeps first theme for duplicate names", () => {
  const name = `plugin-theme-keep-${Date.now()}`
  const one = structuredClone(DEFAULT_THEMES.bioinformatica)
  const two = structuredClone(DEFAULT_THEMES.bioinformatica)
...
test("resolveTheme rejects circular color refs", () => {
  const item = structuredClone(DEFAULT_THEMES.bioinformatica)
