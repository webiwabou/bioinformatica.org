...
const envPath = process.env.BIOINFORMATICA_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".bioinformatica-cli")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@bioinformatica/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "bioinformatica-cli.exe" : "bioinformatica-cli"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right bioinformatica-cli package. Try manually installing " +
