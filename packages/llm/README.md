...
Schema-first LLM core for bioinformatica. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@bioinformatica/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@bioinformatica/llm/providers"
